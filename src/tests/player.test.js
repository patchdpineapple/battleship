...
test("aiAttack() updates player board status",()=>{
    let recordAttack = cpu.aiAttack(player); console.log(recordAttack);
    let attackedCoord = player.board.boardCoordinates.findIndex((coord) => coord.pos.x === recordAttack.x && coord.pos.y === recordAttack.y);
